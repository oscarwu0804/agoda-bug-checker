    ...
    for city in cities:
        ...
        for offset in range(0, 365, 30):
            ...
            try:
                ...
                hotels = soup.find_all(True)
                print(f"🔷 soup.find_all(True) 結果數: {len(hotels)}")

                cards = soup.find_all(class_='PropertyCard')
                print(f"🏨 找到 PropertyCard 數: {len(cards)}")

                if len(cards) == 0:
                    print(f"⚠️ 沒有找到任何飯店卡片，可能 Agoda 改版或封鎖")
                    continue
                ...
                for hotel in cards:
                    try:
                        ...
                        if price < avg_price * 0.8:
                            ...
                            send_gmail("Agoda Bug 價警報", msg_text)
                    except:
                        continue
...
schedule.every(1).minutes.do(check_bug_price)
